...
class ProviderStorage:
    ...
    def __init__(self, data_dir: Path):
        ...
        # Initialize SQLite
        self.db_path = data_dir / "providers.db"
        self._init_db()
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS providers (
                    ...
                    plans_accepted TEXT,  -- JSON array
                    search_zip TEXT,
                    search_radius INTEGER,
                    search_specialty TEXT,
                    scraped_at TEXT,
                    UNIQUE(provider_id, search_zip, search_specialty)
                )
            """)
            
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_providers_zip 
                ON providers(search_zip)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_providers_specialty 
                ON providers(search_specialty)
            """)
            conn.execute("""
                ...
                try:
                    conn.execute("""
                        INSERT OR REPLACE INTO providers (
                            ...
                            specialties, sub_specialties, street, city, state, zip, suite,
                            phone, fax, accepting_new_patients, languages, gender,
                            years_in_practice, education, hospital_affiliations, plans_accepted,
    ...
    def export_to_csv(self, filepath: Optional[Path] = None) -> Path:
        ...
        if not providers:
            ...
            with open(filepath, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['name', 'specialties', 'city', 'state', 'zip', 'phone'])
            return filepath
    ...
    def get_stats(self) -> dict:
        ...
        with sqlite3.connect(self.db_dir) as conn:
            ...
            by_zip = conn.execute(
                "SELECT search_zip, COUNT(*) as count FROM providers GROUP BY search_zip"
            ).fetchall()
            by_specialty = conn.execute(
                "SELECT search_specialty, COUNT(*) as count FROM providers GROUP BY search_specialty"
            ).fetchall()
            
            return {
                'total_providers': total,
                'by_zip': {row[0]: row[1] for row in by_zip if row[0]},
                'by_specialty': {row[0]: row[1] for row in by_specialty if row[0]},
